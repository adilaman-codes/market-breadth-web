...
## AI Expert Insight (optional) — one API key

The top of the site (regime scorecard, KPI tiles, one-line read) is generated by plain
Python/TypeScript rules — free, no AI. The **AI Expert Insight** panel at the bottom of the
page is the only part that uses a language model. It writes one short note per trading day
and keeps a rolling window of the **last 8 days** (oldest auto-deleted), so cost stays to a
fraction of a cent per day.

To turn it on:

1. Get an Anthropic API key from https://console.anthropic.com (Settings → API Keys).
2. On your Mac, create a file `~/MarketBreadth/.ai_env` (next to `.turso_env`):

   ```
   ANTHROPIC_API_KEY=sk-ant-...
   # optional, defaults to a small cheap model:
   ANTHROPIC_MODEL=claude-haiku-4-5-20251001
   ```

3. That's it. `run_daily.sh` now calls `generate_ai_insight.py` after each collection,
   stores the insight locally, and `push_to_cloud.py` syncs the last 8 days to Turso.

If the key is missing the step is skipped silently — the site just shows "no AI insight for
this day" and everything else keeps working. To backfill or regenerate manually:

```bash
cd ~/MarketBreadth && python3 generate_ai_insight.py            # latest day
cd ~/MarketBreadth && python3 generate_ai_insight.py --date 2026-07-02 --force
```

No Vercel change is needed — the website already reads whatever is in Turso.
